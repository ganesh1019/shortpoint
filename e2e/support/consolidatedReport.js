...
let ConsolidatedReport = function () {
    ...
    // This method get all filename from directory path
    function getFileNameListFromDirPath () {
        return new Promise((resolve,reject) => {
            ...
            console.log("Error in getFileNameListFromDirPath " + e);
        });
    }

    // inArray is method which check that value is present in array or not
    ...
    // filesConsolidation method store filename in filesName array if and only if file is unique and then call getURIFileName method
    ...
    // Store URI  in URI array from filesName array file
    ...
    // This method return last modified time of file
    function modifiedTime(fileName, cb) {
      return fs.statSync(fileName).mtime;
    }

    // If multiple file contains same URI value that means test are repeated. So, last file will remain in json_reports folder and old file will be moved to  retry_failed_files folder
